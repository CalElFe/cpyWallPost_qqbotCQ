...
inline namespace CQ {
	...
	//发送好友消息
	//Auth=106 失败返回负值,成功返回消息ID
	整数型 sendPrivateMsg(长整数型 QQ, string&msg);
	...
	//发送群消息
	//Auth=101 失败返回负值,成功返回消息ID
	整数型 sendGroupMsg(长整数型 群号, string&msg);
	...
	//发送讨论组消息
	//Auth=103 失败返回负值,成功返回消息ID
	整数型 sendDiscussMsg(长整数型 讨论组号, string&msg);
	...
	//置群成员名片 Auth=126
	整数型 setGroupCard(长整数型 群号, 长整数型 QQID, string 新名片_昵称);
	...
	//取群列表 Auth=161
	std::map<long long, string> getGroupList();
