...
inline namespace CQ
{
	...
	struct code {
		...
		static string image(string fileurl);

		//[CQ:record,file={1},magic={2}] - 发送语音
		//文件以 酷Q目录\data\record\ 为基础
		static string record(string fileurl, bool magic = false);
		//[CQ:record,file={1},magic={2}] - 发送语音
		//文件以 酷Q目录\data\record\ 为基础
		//此函数将会调用系统语音合成将wantSpeak合成到指定文件
		//static string record(string fileurl, string wantSpeak, bool magic = false);

		//[CQ:face,id={1}] - QQ表情
		static string face(int faceid);

		//[CQ:face,id={1}] - QQ表情
		static string face(CQ::face face);

		//[CQ:at,qq={1}] - @某人
		static string at(long long QQ);

		//[CQ:effect,type=art,id=2003,content=小吉] - 魔法字体
		static string effect(string type, int id, string content);

		//[CQ:sign,title=晒幸福,image=http://pub.idqqimg.com/pc/misc/files/20170825/cc9103d0db0b4dcbb7a17554d227f4d7.jpg] - 签到
		static string sign(string title, string imageUrl);
		...
		static string anonymous(bool ignore);
